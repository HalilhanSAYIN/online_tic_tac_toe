...
class _GameScreenState extends State<GameScreen> {
  ...
  void initState() {
    ...
    _initializeGame();
    _confettiController =
        ConfettiController(duration: const Duration(seconds: 3)); // Süreyi ayarla
    _initializeGame();
  }
      ...
      if (winner.isNotEmpty) {
        ...
        _gameService.gameStream(widget.gameId).first.then((snapshot) {
          snapshot.reference.update({
            'status': 'finished',
            'winner': winner == _playerSymbol
                ? gameData['player1nickname']
                : gameData['player2nickname'],
                'winnericon': winner == _playerSymbol
                ? gameData['player1icon']
                : gameData['player2icon']
          });
        });
...
/*
  ...
  /////
  // Konfeti controller tanımla
  late ConfettiController _confettiController;

 
  @override
  void dispose() {
    _confettiController.dispose(); // Konfeti controller'ı serbest bırak
    super.dispose();
  }
  ///
              ...
              children: [
                Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text("${gameData['winner']} Win!", style: TextStyle(fontSize: 24),),
                      Text("${gameData['winnericon']}", style: TextStyle(fontSize: 100),),
                      ElevatedButton(
                        onPressed: () {
...
class _GameScreenState extends State<GameScreen> {
  ...
  void _handleMove(int row, int col, gameData) async {
    if (_board[row][col] == '' && _turn == _getPlayerTurn(gameData)) {
      ...
      String winner = _gameLogic.checkWinner(_board); // Dinamik checkWinner
      if (winner.isNotEmpty) {
        _showGameEndDialog('$winner Winner!');
        await _gameService.updateBoard(widget.gameId, _board, _turn, row, col);
        _gameService.gameStream(widget.gameId).first.then((snapshot) {
